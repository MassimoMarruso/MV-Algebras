...
lemma le_iff_oMul_not {x y : A} : x ≤ y ↔ x ⊙ (- y) = 0 := by
  calc x ≤ y
...
instance {A : Type*} [MVAlgebra A] : PartialOrder A where
  ...
  le_trans := by
    intro x y z
    rw[le_iff_exists,le_iff_exists,le_iff_exists]
    intro ⟨x',hx⟩ ⟨y',hy⟩
    use x' ⊕ y'
    calc x ⊕ (x' ⊕ y')
  ...
  le_antisymm := by
    intro x y h₁ h₂
    replace h₁ : x ⊙ (- y) = 0 := le_iff_oMul_not.mp h₁
    replace h₂ : x = y ⊕ (x ⊖ y) := le_iff_oAdd_oNeg.mp h₂
    calc x
    _ = y ⊕ (x ⊖ y) := h₂
...
lemma oMul_le_le_not_oAdd (x y z : A) : x ⊙ y ≤ z ↔ x ≤ - y ⊕ z := by
  calc x ⊙ y ≤ z
  _ ↔ (- (x ⊙ y) ⊕ z) = 1 := by rw[le_iff_not_oAdd]
  _ ↔ ((- x ⊕ - y) ⊕ z) = 1 := by rw[not_oMul]
  _ ↔ (- x ⊕ (- y ⊕ z)) = 1 := by rw[oAdd_assoc]
  _ ↔ x ≤ - y ⊕ z := by rw[le_iff_not_oAdd]
...
theorem min_def (x y : A) [MVChain A] : x ⊓ y = if (x ≤ y) then x else y := by
    ...
    case neg h₂ =>
      ...
      _ = y ⊙ (- ( - - y) ⊙ (- x)) := by simp
      _ = y ⊙ (- (y) ⊙ (- x)) := by simp
      _ = y ⊙ (- 0) := by rw[le_iff_oMul_not.mp h]
      _ = y ⊙ 1 := by simp
      _ = y := by simp
